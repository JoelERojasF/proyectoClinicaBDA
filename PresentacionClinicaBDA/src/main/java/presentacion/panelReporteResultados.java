...
public class panelReporteResultados extends javax.swing.JPanel {
    ...
    private void cargarParametrosEnTabla() {
        ...
        for (ParametroEntidad p : parametrosActuales) {
            ...
            System.out.println("--- BUSCANDO RANGO ---");
            System.out.println("Parámetro: " + p.getNombre());
            System.out.println("Edad calculada del cliente: " + edad);
            System.out.println("Sexo del cliente: " + sexo);
            System.out.println("----------------------");
            
            RangoEntidad rango = resultadoBO.obtenerRangoSugerido(p, sexo, edad);
            String rangoTexto = "Sin rango definido";

            if (rango != null) {
                rangoTexto = rango.getRangoInicial() + " - " + rango.getRangoFinal() + " " + p.getUnidadMedida();
            } else {
                if (p.getNombre().equalsIgnoreCase("Colesterol Total")) {
                    rangoTexto = "0.00 - 200.00 mg/dL";
                } else if (p.getNombre().equalsIgnoreCase("Triglicéridos")) {
                    rangoTexto = "0.00 - 150.00 mg/dL";
                } else if (p.getNombre().equalsIgnoreCase("Glucosa")) {
                    rangoTexto = "70.00 - 100.00 mg/dL";
                } else if (p.getNombre().equalsIgnoreCase("Urea")) {
                    rangoTexto = "17.00 - 43.00 mg/dL";
                } else if (p.getNombre().equalsIgnoreCase("Creatinina")) {
                    rangoTexto = (sexo.equalsIgnoreCase("Femenino")) ? "0.60 - 1.10 mg/dL" : "0.70 - 1.30 mg/dL";
                }
            }
